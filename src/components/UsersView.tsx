    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-semibold text-slate-800">{t("users.title")}</h2>
            <p className="text-sm text-slate-500 mt-1">{t("users.description")}</p>
          </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 font-medium">
              <tr>
                ...
                <th className="px-6 py-3">{t("settings.role")}</th>
                <th className="px-6 py-3">{t("users.assignedLocation")}</th>
                <th className="px-6 py-3 text-right">{t("callsTable.actions")}</th>
              </tr>
            </thead>
              ...
              {users.map(u => (
                <tr key={u.email} className="hover:bg-slate-50/50">
                  ...
                  <td className="px-6 py-4 text-slate-500">
                    {u.role === 'admin' ? t('users.allLocations') : (locations.find(l => l.id === u.locationId)?.name || u.locationId || t('users.unassigned'))}
                  </td>
